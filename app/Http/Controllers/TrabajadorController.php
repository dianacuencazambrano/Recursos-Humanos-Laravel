...
class TrabajadorController extends Controller
{
    ...
    public function deleteTrabajador(Request $request)
    {
        try {
            ...
            $response = $response->getBody();
            $response = json_decode($response);
            $msg = ($response == "Eliminación Exitosa") ? $response : 'Error al eliminar';
            return response()->json(['success' => 1, 'message' => $msg], 200);
        } catch (\Exception $th) {
            return response()->json(['success' => 0, 'message' => $th], 201);
        }
    ...
    public function updateTrabajador(Request $request)
    {
        try {
            ...
            $response = $response->getBody();
            $response = json_decode($response);
            $msg = ($response == "Actualización Exitosa") ? $response : 'Actualización Exitosa';
            return response()->json(['success' => 1, 'message' => $msg], 200);
        } catch (\Exception $th) {
            return response()->json(['success' => 0, 'message' => $th], 201);
        }
    ...
    public function validarFechas($fecha){
        $fecha = ($fecha == '' || $fecha == 0) ? '1900-01-01' : $fecha;
        return $fecha;
    }
}
